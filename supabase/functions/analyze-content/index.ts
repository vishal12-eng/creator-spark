...
const TOKEN_COST = 1; // content_analytics costs 1 token

const logStep = (step: string, details?: any) => {
  const detailsStr = details ? ` - ${JSON.stringify(details)}` : '';
  console.log(`[ANALYZE-CONTENT] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an expert YouTube SEO and content optimization specialist. Analyze video content and provide detailed, actionable feedback. Always respond with valid JSON.`;

    const userPrompt = `Analyze this YouTube video content and provide comprehensive optimization recommendations:
...
Provide a JSON response with this exact structure:
{
  ...
  "thumbnailReadability": <number 0-100 based on title clarity>,
  "overallScore": <number 0-100>,
  "issues": [
    {"type": "error" or "warning", "message": "<specific issue>"}
  ],
  "improvements": ["<specific improvement suggestion>"],
  "optimizedTitle": "<improved title with emojis and power words>",
  "optimizedDescription": "<improved description with timestamps, CTAs, and hashtags>"
}`;

    logStep('Calling AI gateway for content analysis...');
